...
def plotCacheStats(system, logfile, loadkey, misskey, cachetype, maxtpbm, keys, maxtpthreads, maxtprps, cmt, save=None):
    ...
    plt.title(system + " " + cachetype + " {} rate with varying RPS".format("refill" if cachetype == "L2D" else "hit"))
    ...
    plt.title(system + " " + cachetype + " Hit Rate with varying keys, connections")
    plt.ylabel("[%]")
    plt.legend()
    plt.tight_layout()
