...
plt.plot([int(k.replace(" rps",'')) for k in rps1_cavium.keys()], list(rps1_cavium.values()), linestyle="--", label=', '.join(rps1_cavium_keys), color="black")
plt.plot([int(k.replace(" rps",'')) for k in rps2_cavium.keys()], list(rps2_cavium.values()), linestyle=":", label=', '.join(rps2_cavium_keys), color="black")
plt.plot([int(k.replace(" rps",'')) for k in rps1_cavium.keys()], avg_cavium, label='Average', linestyle="-", color="red")
plt.legend()
plt.xlabel("RPS")
...
plt.figure(figsize=(9, 6))
plt.title("Average Total Outstanding Client Requests, x86 (Xeon)")
plt.plot([int(k.replace(" rps",'')) for k in rps1_xeon.keys()], list(rps1_xeon.values()),linestyle="--", label=', '.join(rps1_xeon_keys), color="black")
plt.plot([int(k.replace(" rps",'')) for k in rps2_xeon.keys()], list(rps2_xeon.values()), linestyle=":", label=', '.join(rps2_xeon_keys), color="black")
plt.plot([int(k.replace(" rps",'')) for k in rps1_xeon.keys()], avg_xeon, label='Average', linestyle="-", color="red")
plt.legend()
plt.xlabel("RPS")
plt.ylabel("Outstanding Requests")
plt.tight_layout()
...
plt.plot([int(k.replace(" rps",'')) for k in rps2_cavium.keys()], avg_cavium, label='ARM (Cavium)')
plt.axvline(159000,linestyle='--', color='black')
plt.plot([int(k.replace(" rps",'')) for k in rps2_xeon.keys()], avg_xeon, label='x86 (Xeon)')
